...
func (m *tlsManager) getTLSSettings(dsInfo sqleng.DataSourceInfo) (tlsSettings, error) {
	...
	if isTLSDisabled {
		m.logger.Debug("Turbo TLS/SSL is disabled")
		return tlsconfig, nil
	}

	m.logger.Debug("Turbo TLS/SSL is enabled", "tlsMode", tlsconfig.Mode)
...
func writeCertFile(logger log.Logger, fileContent string, generatedFilePath string) error {
	...
	if fileContent != "" {
		...
		// Make sure the file has the permissions expected by the Turbo driver, otherwise it will bail
