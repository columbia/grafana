...
type turboMacroEngine struct {
	*sqleng.SQLMacroEngineBase
	timescaledb bool
}

func newTurboMacroEngine(timescaledb bool) sqleng.SQLMacroEngine {
	return &turboMacroEngine{
		SQLMacroEngineBase: sqleng.NewSQLMacroEngineBase(),
		timescaledb:        timescaledb,
	}
}

func (m *turboMacroEngine) Interpolate(query *backend.DataQuery, timeRange backend.TimeRange, sql string) (string, error) {
	// TODO: Handle error
...
//nolint:gocyclo
func (m *turboMacroEngine) evaluateMacro(timeRange backend.TimeRange, query *backend.DataQuery, name string, args []string) (string, error) {
	switch name {
	case "__time":
		if len(args) == 0 {
