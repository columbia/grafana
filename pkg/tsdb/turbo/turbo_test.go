...
// To run this test, set runTurboTests=true
// Or from the commandline: GRAFANA_TEST_DB=turbo go test -tags=integration -v ./pkg/tsdb/turbo
// The tests require a PostgreSQL db named grafanadstest and a user/password grafanatest/grafanatest!
// Use the docker/blocks/turbo_tests/docker-compose.yaml to spin up a
// preconfigured Turbo server suitable for running these tests.
...
func TestIntegrationTurbo(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test")
	}
	// change to true to run the PostgreSQL tests
	const runTurboTests = false

	if !(db.IsTestDbTurbo() || runTurboTests) {
		t.Skip()
	}
	...
	exe, err := sqleng.NewQueryDataHandler(config, &queryResultTransformer, newTurboMacroEngine(dsInfo.JsonData.Timescaledb),
		logger)
	...
	t.Run("Given a table with different native data types", func(t *testing.T) {
		...
		sql = `
			INSERT INTO turbo_types VALUES(
				1,2,3,
				4.5,6.7,1.1,1.2,
		...
		t.Run("When doing a table query should map Turbo column types to Go types", func(t *testing.T) {
			query := &backend.QueryDataRequest{
				Queries: []backend.DataQuery{
					{
						JSON: []byte(`{
							"rawSql": "SELECT * FROM turbo_types",
							"format": "table"
						}`),
						RefID: "A",
					},
	...
	t.Run("Given a table with event data", func(t *testing.T) {
		...
		t.Run("When row limit set to 1", func(t *testing.T) {
			...
			handler, err := sqleng.NewQueryDataHandler(config, &queryResultTransformer, newTurboMacroEngine(false), logger)
			require.NoError(t, err)
...
func InitTurboTestDB(t *testing.T) *xorm.Engine {
	testDB := sqlutil.TurboTestDB()
	x, err := xorm.NewEngine(testDB.DriverName, strings.Replace(testDB.ConnStr, "dbname=grafanatest",
		"dbname=grafanadstest", 1))
	require.NoError(t, err, "Failed to init turbo DB")
